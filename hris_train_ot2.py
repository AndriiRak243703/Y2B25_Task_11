...
class OT2Env(gym.Env):
    ...
    def step(self, action):
        ...
        proximity = min(1.0 / (curr_dist + 0.01), 50.0) 
...
def main():
    ...
    # This prevents PyTorch from fighting for resources with PyBullet
    th.set_num_threads(4) 

    task = Task.init(
        project_name='Mentor Group - Myrthe/Group 1', 
        task_name='hris_Precision_Final_CPU_OPT',
        output_uri=True 
    )
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix='ot2_curriculum_v5'
    )

    # 3. HYPERPARAMETER TWEAKS FOR SPEED
    model = PPO(
        "MlpPolicy",
        env,
        device="cpu",      # <--- CRITICAL: Force CPU to avoid transfer overhead
        learning_rate=2.5e-4,
        n_steps=2048,
        batch_size=256,    # <--- SPEED UP: Bigger batches = fewer updates/sec
        n_epochs=4,        # <--- SPEED UP: Don't over-train, just move on
        gamma=0.995,
        ent_coef=0.001,
        verbose=1,
    ...
    try:
        ...
        model.save("final_model_v5")
        task.upload_artifact("final_model_v5", "final_model_v5.zip")
